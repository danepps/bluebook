...
- **Subsequent-cite et-al threshold is lower than first-cite — experimental-only, under test.** **(experimental file only)** — `<citation>` carries `et-al-subsequent-min="4" et-al-subsequent-use-first="1"` alongside the first-cite `et-al-min="5" et-al-use-first="1"`. Effect: a four-author work lists all four on the *first* cite but collapses to `Author1 et al.` on *supra* repeats; 5+ authors abbreviate in both positions; 2–3 author works list fully in both. `et-al-subsequent-min`/`-use-first` are **global name options** (declarable only on `<style>`/`<citation>`/`<bibliography>`, never on `<name>`), so they apply to every subsequent name list — but in this style the only subsequent author *lists* are the `supra` short forms (books/journals/reports/etc.); case subsequent cites use Rule 10.9 short title (or `supra` with a single party name), so this is effectively secondary-source-only in practice. **Caveat:** like all position-based logic here, depends on the processor seeing surrounding notes (reliable in a live document, not in copied-out text). Not yet promoted to the stable file.
