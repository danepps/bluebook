...
6. **Bump the README's "Style last updated" date on every base-style PR.** Author preference: whenever a PR changes `BluebookDSEStyle.csl`, update the human-readable date in the blockquote at the top of `README.md` (line ~5, `**Style last updated: …**`) to the PR date, in the same PR. This is separate from the CSL's `<updated>` element (auto-rewritten by `bump-updated.yml`) — the README date is *not* automated. README-only PRs don't need it.
...
- **Statutes (Rule 12) (experimental file only, under test)** — Zotero Statute item. Conventions: type the act name in full in **Name of Act** (including "of 1990" when official — the old auto-appended ` of YYYY` is gone); **Public Law Number** gets a `Pub. L. No. ` prefix from the style (citeproc renders its hyphen as an en dash — cosmetic, not worth fighting); **Code**/**Code Number** → reporter/volume; **Section** renders via the section symbol term (auto-pluralizes to `§§` for ranges); **Pages** set ⇒ session-law form (`Title, Pub. L. No. N, § S, 124 Stat. 119 (Year)`, section before the volume); **Date Enacted** → year parenthetical — leave empty for the current official U.S.C. (Rule 12.3.2, 21st ed.), fill for state/unofficial codes and session laws. Publisher parentheticals for unofficial codes (`(West 2020)`) are **not supported**; the author never cites statutes, so this is basic compliance only — don't over-invest.
...
- **Experimental file was re-baselined from stable** (June 2026) and is accumulating the compliance-queue fixes, to be **promoted to stable as one batch** when the author calls it done (author's decision, June 2026). Currently carries: locator-label handling (Rules 3.3–3.4; Zotero-validated), authorless supra short-title fallback (Rule 4.2; Zotero-validated), the Edition-field `ed.` guard (Rule 15.4; Zotero-validated — has a promotion-time README task), and the statute branch rewrite (Rule 12; awaiting spot-check). See design choices for all four.
...
3. ~~Statutes (Rule 12): `legislation` branch emits no date parenthetical (still required for state/unofficial codes) and no `Stat.` page for session laws; `§` is hardcoded singular (no `§§`)~~ — in experimental, under test.
...
- `<updated>` will be rewritten automatically by the workflow; don't bother editing it manually. **But do** hand-update the README's "Style last updated" date (top blockquote) — that one isn't automated (see workflow convention #6).
