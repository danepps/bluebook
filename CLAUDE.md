...
- **Experimental file was re-baselined from stable** (June 2026) and is accumulating the compliance-queue fixes, to be **promoted to stable as one batch** when the author calls it done (author's decision, June 2026). Currently carries: locator-label handling (Rules 3.3–3.4; Zotero-validated) and authorless supra short-title fallback (Rule 4.2; awaiting spot-check). See design choices for both.
