...
- `BluebookDSEStyle-EtAl.csl` — a **side variant** of the stable style with different et-al thresholds (see design choices): et-al kicks in only at **10+** authors on first appearance and **4+** on subsequent appearance, vs. the stable style's uniform 5. Intentionally shares main's **`<title>`, `<id>`, and `<link rel="self">`** (a personal hand-off for a friend who wants it to appear as the ordinary "Bluebook Style — Epps Version" in Zotero) — so the **only** difference from stable is the five et-al attributes. Because the `<id>` collides with stable, Zotero treats the two as the same style: don't install both in one library, and think twice before merging this to `main` as a public style. When the stable style changes, re-sync this file. Not in the README.
...
- **`et-al-min="5"`** — the author prefers listing up to four names before abbreviating, even though Bluebook Rules 15.1/16.1 specify three. (The `BluebookDSEStyle-EtAl.csl` side variant overrides this — see below.)
- **Et-al side variant (`BluebookDSEStyle-EtAl.csl`)** — a standalone style file, identical to stable (including `<title>`/`<id>`/`<link rel="self">`, deliberately, so it presents as the ordinary style in a friend's Zotero — note the shared `<id>` means Zotero treats it as the same style as stable) except for the et-al thresholds: `<citation>` carries `et-al-min="10" et-al-use-first="1" et-al-subsequent-min="4" et-al-subsequent-use-first="1"`, so "et al." only appears at **10+** authors on first appearance and **4+** on subsequent appearance (lists up to 9 / up to 3 respectively before truncating; first author + "et al." when it does). `<bibliography>` uses `et-al-min="10"` (a bibliography entry is a full listing, so the first-appearance threshold applies). `et-al-use-first` stays `1` to match the base style / Bluebook 15.1–16.1. Only affects multi-author books/articles — cases have no author. Keep in sync with stable when the shared style changes (re-copy stable, re-apply these five attribute values). It is registered in `.github/workflows/bump-updated.yml`'s `paths:` list so its `<updated>` auto-bumps like the others.
...
- **Case name honors "Omit Author" (July 2026)** — the full/first case cite in `source` renders the case name via `<text macro="author"/>` (for `legal_case`, a `<names variable="author">` node whose substitute is `<text variable="title"/>`) instead of a bare `<text variable="title"/>`. Because the name now flows through a `names` node, Zotero's **Omit Author** (`suppress-author`) drops it — leaving `410 U.S. 113 (1973)` when the party names already appear in the sentence (Rule 10.9 grammatical use). Output is byte-identical when *not* suppressed: a case has no `author`, so the title substitute always fires. The short/subsequent form already suppressed the name via `name-short-macro`, so full and short forms are now consistent. **Related data-entry note:** there is *no* CSL way to suppress the court name only for the U.S. Supreme Court (CSL can't inspect field text) — the fix is to leave the Zotero **Court** field blank for SCOTUS (Rule 10.4(a); the `U.S.` reporter conveys the court). Documented in the README case section.
...
- **Locator labels (Rules 3.3–3.4)** — `at` introduces only *page* pincites (`Id. at 5`); any other locator label renders via its symbol term with no `at` (`Id. § 4`, `Id. ¶¶ 12–13` — citeproc pluralizes the symbol for ranges; labels without a symbol form fall back to short form, e.g. `ch.`). All bare pincite points in `source` (journal, case, article, report, thesis, chapter, book, generic) route through a shared `pincite` macro with the same page-vs-symbol logic, so non-page locators gain their symbol there too (`TITLE § 5 (2010)`); page locators are unchanged everywhere. Podcast timestamps keep `at TT:TT` via the default *page* label. The two `(manuscript at X)` locators are intentionally left raw.
- **Authorless supra short forms (Rule 4.2)** — when an item has no author, the subsequent-cite chooser now falls back to a short title before *supra* instead of emitting a bare `supra note N`. Font tracks the full-cite rendering of each type: small caps for `book`/`graphic`/`motion_picture`/`report`, plain roman for the unpublished types (`article`/`manuscript`/`thesis`/`document`), italic for everything else (newspaper, magazine, webpage, blog, chapter, generic). The existing `article-journal` short-title and podcast host/show fallbacks are unchanged and take precedence.
- **Edition field `ed.` guard (Rule 15.4)** — the issuance macro appends the `ed.` label only when the Edition field is numeric per CSL's `is-numeric` test (`3`, `3d`, `11th`); any other value renders **verbatim** (`rev. ed.`, `2d rev. ed.`). CSL cannot string-match field content, so non-numeric values must contain the complete Bluebook text. The README's Edition note and a bare-`rev.` migration warning were updated at promotion time.
- **Statutes (Rule 12)** — Zotero Statute item. Conventions: type the act name in full in **Name of Act** (including "of 1990" when official — the old auto-appended ` of YYYY` is gone); **Public Law Number** gets a `Pub. L. No. ` prefix from the style (citeproc renders its hyphen as an en dash — cosmetic, not worth fighting); **Code**/**Code Number** → reporter/volume; **Section** renders via the section symbol term (auto-pluralizes to `§§` for ranges); **Pages** set ⇒ session-law form (`Title, Pub. L. No. N, § S, 124 Stat. 119 (Year)`, section before the volume); **Date Enacted** → year parenthetical — leave empty for the current official U.S.C. (Rule 12.3.2, 21st ed.), fill for state/unofficial codes and session laws. Pre-1957 chaptered session laws (Rule 12.4) take `ch. N` instead of a public law number: when **Public Law Number** is empty the style falls back to `chapter-number` (rendered with a `ch. ` prefix), which has no native Zotero Statute field, so set `chapter-number: N` in the **Extra** field. Publisher parentheticals for unofficial codes (`(West 2020)`) are **not supported**; the author never cites statutes, so this is basic compliance only — don't over-invest.
...
- **License is CC BY-SA 4.0** (June 2026) — upgraded from upstream's 3.0, which §4(b) of BY-SA 3.0 permits for adaptations, so that GitHub's license detection names it instead of showing "Other" (GitHub's dataset has no CC 3.0 variants). Declared in three places that must stay in sync: `LICENSE` (canonical choosealicense text — don't reformat, detection is similarity-based), both CSL files' `<rights>`, and the README credits line.
...
- **The compliance-queue batch was promoted to stable** (June 2026): locator-label handling (Rules 3.3–3.4), authorless supra short-title fallback (Rule 4.2), the Edition-field `ed.` guard (Rule 15.4, with the README Edition note + bare-`rev.` migration warning updated), and the statute branch rewrite + chaptered session laws (Rule 12). Stable and experimental are now identical apart from `<title>`/`<id>`/`<link rel="self">`; the experimental file is a clean baseline for the next batch. See design choices for all four.
- **"Omit Author" now suppresses the case name (July 2026)** — promoted to stable: the full case cite routes its name through the `author` macro so Zotero's Omit Author drops the case name (Rule 10.9 grammatical use). Stable and experimental are identical again apart from `<title>`/`<id>`/`<link rel="self">`. See design choices. The companion README note on leaving the **Court** field blank for SCOTUS (Rule 10.4(a)) is the data-entry counterpart — court-name suppression can't be done in pure CSL.
...
1. ~~Locator labels (Rules 3.3–3.4): non-page pincites rendered `at N` instead of `§ N` / `¶ N`~~ — **shipped to stable (June 2026).**
2. ~~Authorless newspaper/magazine/webpage subsequent cites render a bare `supra note N` — need a short-title fallback before *supra* (Rule 4.2)~~ — **shipped to stable (June 2026).**
3. ~~Statutes (Rule 12): `legislation` branch emits no date parenthetical (still required for state/unofficial codes) and no `Stat.` page for session laws; `§` is hardcoded singular (no `§§`)~~ — **shipped to stable (June 2026), incl. chaptered session laws (`ch. N`).**
