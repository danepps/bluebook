...
- **Experimental file was re-baselined from stable** (June 2026) and now carries one change under test: locator-label handling (Rules 3.3–3.4; see design choices). Awaiting Zotero spot-check before promotion.
...
2. Authorless newspaper/magazine/webpage subsequent cites render a bare `supra note N` — need a short-title fallback before *supra* (Rule 4.2); only `article-journal` and podcasts have one today.
