...
- `BluebookDSEStyle-EtAl.csl` — a **side variant** of the stable style with different et-al thresholds (see design choices): et-al kicks in only at **10+** authors on first appearance and **4+** on subsequent appearance, vs. the stable style's uniform 5. Otherwise a byte-for-byte copy of stable (apart from `<title>`/`<id>`/`<link rel="self">`); when the stable style changes, re-sync this file the same way experimental is promoted. Not yet documented in the README (installable directly from its Pages URL).
...
- **Et-al side variant (`BluebookDSEStyle-EtAl.csl`)** — a standalone style file, identical to stable except for `<title>`/`<id>`/`<link rel="self">` and the et-al thresholds: `<citation>` carries `et-al-min="10" et-al-use-first="1" et-al-subsequent-min="4" et-al-subsequent-use-first="1"`, so "et al." only appears at **10+** authors on first appearance and **4+** on subsequent appearance (lists up to 9 / up to 3 respectively before truncating; first author + "et al." when it does). `<bibliography>` uses `et-al-min="10"` (a bibliography entry is a full listing, so the first-appearance threshold applies). `et-al-use-first` stays `1` to match the base style / Bluebook 15.1–16.1. Only affects multi-author books/articles — cases have no author. Keep in sync with stable when the shared style changes (re-copy stable, re-apply these five attribute values). It is registered in `.github/workflows/bump-updated.yml`'s `paths:` list so its `<updated>` auto-bumps like the others.
