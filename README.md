...
> **Note on signals.** Bluebook introductory signals (*See*, *See also*, *Cf.*, *But see*, etc.) are **not** produced by this style. They are inserted by a companion Zotero plug-in: **[Bluebook Signals](https://danepps.github.io/zotero/bluebook-signals/)** — install that alongside this style if you use signals.

> **Note on citation fixes.** Some Bluebook rules can't be expressed in a pure CSL style — `[hereinafter]` short forms, dropping the redundant `(YYYY)` when a journal volume is a year, forcing `, at` before a pincite when a title ends in a numeral. Those are applied by the companion **[Bluebook Citations Fixer](https://danepps.github.io/zotero/bluebook-citations-fixer/)** plug-in, which post-processes this style's output — install it alongside this style for the full feature set. Every gap listed under [Known limitations](#known-limitations) says whether the Fixer closes it.
...
## Importing sources from databases (JSTOR, HeinOnline, etc.)

Most legal and academic sources live behind an institutional subscription, so the import workflow has a couple of wrinkles worth knowing.

- **Log in through your institution's proxy first.** You need an authenticated session with the database (via your library's proxy / single sign-on) before Zotero can grab anything. **You only have to teach Zotero the proxy once** — the Zotero connector detects the proxied address the first time you save from a site and will automatically route that site through your proxy on every later visit.
- **JSTOR:** once per session you have to **manually click Download and accept the terms** on JSTOR before it will hand over PDFs. After you've done that once in a session, the connector will pull the PDF automatically for subsequent saves; until you do, you'll get the metadata but no attached PDF.
- **HeinOnline:** PDF retrieval is **a little unreliable** — sometimes the connector grabs the PDF, sometimes it doesn't, and you may have to re-save or download the PDF by hand. Hein also **usually imports the year wrong**, as the *range* of years the volume was published (e.g., `2011-2012`) rather than the single year you want. **Fix the Date field manually** after importing.

> **Always double-check `Journal Abbr` on imported items.** No matter which database a journal article comes from, verify the **Journal Abbr** field holds the correct Bluebook abbreviation (e.g., `Harv. L. Rev.`) before you cite — importers often leave it blank or fill it from a non-Bluebook scheme. See the [Journal article](#journal-article) entry and the [Journal abbreviations](#how-the-style-works-things-everyone-should-know) note for details.
...
- **No introductory signals.** *See*, *Cf.*, *But see*, etc. are not produced by this style. **Fix:** the **[Bluebook Signals](https://danepps.github.io/zotero/bluebook-signals/)** plug-in inserts them (Ctrl+S in the citation dialog's Prefix field).
...
- **No `[hereinafter shortname]` support.** Bluebook lets authors coin a custom short form on first citation (e.g., `[hereinafter Reimagining]`) and then use that short form on every subsequent cite. CSL has no native concept of author-defined short forms, so the style can't generate the `[hereinafter …]` marker or honor it on subsequent cites. **Fix:** the **[Bluebook Citations Fixer](https://danepps.github.io/zotero/bluebook-citations-fixer/)** adds `[hereinafter Author, Short Title]` to the first full cite of an ambiguous same-author work and inserts the short title before `supra note` on later cites (Rule 4.2(b)); it applies on a full Zotero refresh.
- **Can't detect "volume number is a year."** When a journal's volume *is* a year (e.g., `2024 Wis. L. Rev. 501`), Bluebook suppresses the redundant trailing `(2024)`. This style renders both (`2024 Wis. L. Rev. 501 (2024)`), because CSL can't introspect the volume to tell whether it looks like a year. **Fix:** the **Bluebook Citations Fixer** drops the trailing `(YYYY)` when the volume is a four-digit year (and does the same for statutes whose name already ends in the year, Rule 12.3.2).
- **Can't detect "title ends in a numeral."** When a book or report title ends in a number (e.g., *Article III* at 45), Bluebook requires the pincite be written `, at 45` to avoid running the two numbers together. This style just emits the page number, so you may get ambiguous output like `ARTICLE III 45`. **Fix:** the **Bluebook Citations Fixer** inserts the `, at` automatically when a title ends in a numeral.
- **Cases: five-footnote rule has limits.** Bluebook Rule 10.9 says a short-form case cite (`*Brown*, 347 U.S. at 495`) may be used only if the full cite appears in the same footnote or one of the preceding five footnotes — otherwise, cite in full again. The style now applies this automatically via Zotero's footnote-distance tracking: cases revert to a full cite once the prior cite falls outside the five-footnote window. The catch is that this depends on the processor seeing the surrounding notes — it's reliable in a live document but not when citation text is copied out of context, and it counts footnote distance rather than the exact "same-or-preceding-five" set Bluebook describes. For full control, many authors (including this one) still enter cases and case short forms manually in the footnote text rather than via Zotero. If you mix hand-typed cites with Zotero cites, Zotero can't see the typed ones and may wrongly render the next Zotero cite of the same source as `Id.` — the **Bluebook Citations Fixer**'s **"Break id."** checkbox (in the citation dialog, under *Omit Author*) rewrites that `Id.` into the correct short form.

> **Companion plug-in.** Every gap marked **Fix** above — hereinafter, volume-as-year, title-ends-in-numeral, and the "Break id." case — is closed by the **[Bluebook Citations Fixer](https://danepps.github.io/zotero/bluebook-citations-fixer/)** plug-in; signals come from **[Bluebook Signals](https://danepps.github.io/zotero/bluebook-signals/)**. A plug-in can introspect the data and post-process Zotero's citation output in ways that a pure CSL file cannot. Both install via **Tools → Plugins → gear menu → Install Plugin From File**.
